...
#define NFS_BLKS_SZ(blks)               (2 * blks * NFS_IO_SZ())
...
#define NFS_DATA_OFS(dat)               (nfs_super.data_offset + NFS_BLKS_SZ(dat))
...
struct nfs_inode
{
    ...
    uint8_t*           data;           
    // int                dat[nfs_DATA_PER_FILE]; //该inode对应文件占用的数据块在data位图中的下标
    int                dat;
};  
